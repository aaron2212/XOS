...
DIR *opendir(char *dirname)
{
    ...
    if (strcmp(dirname, PATH_SEPARATOR) == 0)
    {
        ...
        for (unsigned int i = 0; i < total_rootfs_dirs + total_rootfs_files; i++)
        {
            ...
            char *entry_name = kmalloc(strlen(entry_names[i]));
            ...
            if (strchr(entry_name, '/') == NULL)
            {
                ...
                dirent->name = kmalloc(strlen(entry_name));
...
{
    kprintf("%x:%x\n", dir->current_dir, dir->total_entries);
...
// Attempt to remove (delete) a file from the filesystem
...
void find_all_entries(struct entry entries[])
{
    ...
    if (*pos == STARTING_MARKER_ENTRY)
    {
        ...
        while (cur_entry < (total_rootfs_entries))
        { // total_entries-1
